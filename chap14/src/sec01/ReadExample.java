...
//		}
		/*
		int data = is.read();		//test.db안의 1바이트 읽어 리턴, 만약에 못 읽으면-1을 리턴
		System.out.println("1byte -> "+ data1);		//10
		int data2 = is.read(); //test1.db안의 1바이트 읽어 리턴, 만약에 못 읽으면 -1을 리턴
		System.out.println("\"1byte -> \"+ data2");	//20
		*/
		
//		InputStream is = new FileInputStream("./bin/sec01/test1.db");
//		byte[] buffer = new byte[3];
//		int readByteNum = is.read(buffer);	//buffer 크기만큼 test.db에 있는 데이터를 가져온다.
